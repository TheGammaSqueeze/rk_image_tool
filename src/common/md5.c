/*
 * Public domain MD5 implementation, adapted from RSA Data Security Inc.
 * reference implementation and common public-domain derivatives.
 */
#include "md5.h"
#include <string.h>
...
#define FF(a,b,c,d,x,s,ac) do { (a) += F((b),(c),(d)) + (x) + (uint32_t)(ac); (a) = ROTL((a),(s)); (a) += (b); } while(0)
#define GG(a,b,c,d,x,s,ac) do { (a) += G((b),(c),(d)) + (x) + (uint32_t)(ac); (a) = ROTL((a),(s)); (a) += (b); } while(0)
#define HH(a,b,c,d,x,s,ac) do { (a) += H((b),(c),(d)) + (x) + (uint32_t)(ac); (a) = ROTL((a),(s)); (a) += (b); } while(0)
#define II(a,b,c,d,x,s,ac) do { (a) += I((b),(c),(d)) + (x) + (uint32_t)(ac); (a) = ROTL((a),(s)); (a) += (b); } while(0)
...
void md5_final(struct md5_ctx *ctx, uint8_t digest[16])
{
    ...
    for (int i = 0; i < 8; ++i) bits[i] = (uint8_t)(count >> (i*8));
    size_t idx = (size_t)((ctx->count >> 3) & 0x3F);
